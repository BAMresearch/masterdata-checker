...
def check_instance():
    ...
    info.append(f"Total number of EXPERIMENT TYPES: {str(o.get_experiment_types().totalCount)}\n")
    ...
    info.append(f"Total number of OBJECT TYPES: {str(o.get_object_types().totalCount)}\n")
    ...
    info.append(f"Total number of MATERIAL TYPES: {str(o.get_material_types().totalCount)}\n")
    ...
    info.append(f"Total number of DATASET TYPES: {str(o.get_dataset_types().totalCount)}\n")
    
    for dataset in o.get_dataset_types():
        info.append(f"  {dataset}")
        
    info.append(f"\nListing VOCABULARIES in {instance}\n")
    info.append(f"Total number of VOCABULARIES: {str(o.get_vocabularies().totalCount)}\n")
