    ...
    for term in expected_terms:
        if term not in row_headers:
            ...
            if term == 'Version':
                ...
                non_integer_indices = [
                    i + 5
                    for i, cell in enumerate(column_below_version)
                    if not (str(cell).isnumeric() or '$' in str(cell))
                ]
            ...
            # Check the column below "Code"
                ...
                invalid_codes = [
                    i + 5
                    for i, cell in enumerate(column_below_code)
                    if not (
                        re.match(r'^\$?[A-Z0-9_.]+$', str(cell)) or '$' in str(cell)
                    )
                ]
                if invalid_codes:
                ...
                if len(set(column_below_code)) != len(column_below_code):
                    ...
                    for prop in column_below_code:
                        if prop in seen_props:
                            ...
                            seen_props.add(prop)
                    errors.append(
                        f'Error: The following properties are repeated: {repeated_props}. Please, delete the duplicates, and leave just one occurence'
                    )
            ...
            # Check the cell below "Description"
                ...
                invalid_indices = [
                    i + 5
                    for i, cell in enumerate(column_below_description)
                    if not (re.match(r'.*//.*', str(cell)) or '$' in str(cell))
                ]
                if invalid_indices:
                    errors.append(
                        f"Error: Invalid value(s) found in the '{term}' column at row(s): {', '.join(map(str, invalid_indices))}. Description should follow the schema: English Description + '//' + German Description."
                    )
            ...
            # Check the cell below "Mandatory"
                ...
                invalid_mandatory = [
                    i + 5
                    for i, cell in enumerate(column_below_mandatory)
                    if (cell not in ['TRUE', 'FALSE'] and '$' not in str(cell))
                ]
            ...
            # Check the cell below "Show in edit views"
                ...
                invalid_show = [
                    i + 5
                    for i, cell in enumerate(column_below_show)
                    if (cell not in ['TRUE', 'FALSE'] and '$' not in str(cell))
                ]
                if invalid_show:
                    errors.append(
            ...
            # Check the cell below "Section"
                ...
                for cell in sheet[term_letter][4:]:
                    if cell.value is not None:
                        column_below_section.append(
                            cell.value
                        ) if '$' not in cell.value else column_below_section.append(
                            cell.value.replace('$', '')
                        )
                    else:
                        pass
                ...
                invalid_section = [
                    i + 5
                    for i, cell in enumerate(column_below_section)
                    if not (
                        re.match(r'^[A-Z][A-Za-z]*(?:\s[A-Z][A-Za-z]*)*$', str(cell))
                        or '$' in str(cell)
                    )
                ]
                if invalid_section:
                    errors.append(
                        f"Error: Invalid value found in the '{term}' column at row(s): {', '.join(map(str, invalid_section))}. Each word in the Section should start with a capital letter."
                    )
                ...
                for i, current_value in enumerate(column_below_section):
                    ...
                    seen_sections[current_value] = (
                        i  # Update the last seen row index for the current value
                    )

                if non_contiguous_rows:
                    errors.append(
                        f"Error: Non-contiguous rows found for the same 'Section' value at row(s): {', '.join(map(str, non_contiguous_rows))}. Ensure that all properties within the same Section are grouped together."
                    )
                ...
                additional_info_seen = (
                    False  # Flag to track if "Additional Information" has been seen
                )
                ...
                for i, section in enumerate(column_below_section):
                    if section in predefined_section_order:
                        if section == 'General Information':
                            if previous_section_type not in [
                                None,
                                'General Information',
                            ]:
                                section_errors.append(
                                    f"Error at row {i + 5}: 'General Information' should only appear at the beginning."
                                )
                            ...
                            if previous_section_type not in [
                                ...
                                section_errors.append(
                                    f"Error at row {i + 5}: 'Additional Information' should appear after 'General Information' and any user-defined sections."
                                )
                            ...
                            if previous_section_type not in [
                                ...
                                section_errors.append(
                                    f"Error at row {i + 5}: 'Comments' should appear after 'Additional Information'."
                                )
                            comments_seen = (
                                True  # Mark that "Comments" has been encountered
                            )
                        ...
                        if comments_seen:
                            section_errors.append(
                                f"Error at row {i + 5}: User-defined section '{section}' cannot appear after 'Comments'."
                            )
                        if additional_info_seen and not comments_seen:
                            section_errors.append(
                                f"Error at row {i + 5}: User-defined section '{section}' cannot appear after 'Additional Information' but before 'Comments'."
                            )
            ...
            # Check the cell below "Property label"
                ...
                if 'Section' in row_headers:
                    ...
                    column_below_section = [
                        cell.value for cell in sheet[section_letter][4:]
                    ]
                    ...
                    for i, label_value in enumerate(column_below_label):
                        if label_value == 'Notes':
                            ...
                            if section_value != 'Additional Information':
                                errors.append(
                                    f"Error: 'Notes' found in the 'Property label' column at row {i + 5}, but corresponding 'Section' column does not contain 'Additional Information'. Value found: {section_value}"
                                )
            ...
            # Check the cell below "Data type"
                ...
                invalid_type = [
                    i + 5
                    for i, cell in enumerate(column_below_type)
                    if (
                        cell
                        not in [
                            'INTEGER',
            ...
            # Check the column below "Vocabulary code"
                ...
                invalid_vocab = [
                    i + 5
                    for i, cell in enumerate(column_below_vocab)
                    if cell.value
                    and not (
                        re.match(r'^\$?[A-Z0-9_.]', str(cell.value))
                        or '$' not in str(cell)
                    )
                ]
                if invalid_vocab:
...
def check_vocab_terms(sheet, errors):
    expected_terms = ['Version', 'Code', 'Label' 'Description']
    row_headers = [cell.value for cell in sheet[4]]
    for term in expected_terms:
        if term not in row_headers:
                ...
                if len(set(column_below_code)) != len(column_below_code):
                    ...
                    for term in column_below_code:
                        if term in seen_terms:
                            ...
                            seen_terms.add(term)
                    errors.append(
                        f'Error: The following vocabulary terms are repeated: {repeated_terms}. Please, delete the duplicates, and leave just one occurence'
                    )
            ...
            # Check the cell below "Description"
                ...
                invalid_description = [
                    i + 5
                    for i, cell in enumerate(column_below_description)
                    if cell.value and not re.match(r'.*//.*', str(cell.value))
                ]
                if invalid_description:
                    errors.append(
                        f"Error: Invalid value(s) found in the '{term}' column at row(s): {', '.join(map(str, invalid_description))}. Description should follow the schema: English Description + '//' + German Description."
                    )
            ...
            # Check the cell below "Mandatory"
                ...
                if invalid_label:
                    errors.append(
                        f"Error: Invalid value found in the '{term}' column at row(s): {', '.join(map(str, invalid_label))}. Specify the label as text format"
                    )
...
def content_checker(file_path, name_ok):
    ...
    if cell_value_A1 not in entity_types:
        errors.append(
            'The entity type (cell A1) should be one of the following: SAMPLE_TYPE, EXPERIMENT_TYPE, DATASET_TYPE, PROPERTY_TYPE, VOCABULARY_TYPE'
        )
        return '\n'.join(errors)
    else:
                ...
                if term not in second_row_values:
                    ...
                    if term == 'Version':
                        ...
                        if str(cell_below_version.value) != version[1:]:
                            errors.append(
                                'Error: The version should be the same one indicated in the file name'
                            )
                    ...
                    # Check the cell below "Code"
                        ...
                        if cell_below_code.value != code:
                            errors.append(
                                'Error: The code should be the same one indicated in the file name'
                            )

                    # Check the cell below "Description"
                    elif term == 'Description':
                        cell_below_description = sheet.cell(
                            row=3, column=term_index + 1
                        )
                        description_pattern = re.compile(r'.*//.*')
                        if not description_pattern.match(cell_below_description.value):
                            errors.append(
                                "Error: Description should follow the schema: English Description + '//' + German Description."
                            )

                    # Check the cell below "Generated code prefix"
                    elif term == 'Generated code prefix':
                        cell_below_generated_code = sheet.cell(
                            row=3, column=term_index + 1
                        )
                    ...
                    # Check the cell below "Validation script"
                        ...
                        if cell_below_validation.value and not validation_pattern.match(
                            cell_below_validation.value
                        ):
                            errors.append(
                                "Error: Validation script should follow the schema: Words and/or numbers separated by '_' and ending in '.py'"
                            )

                    # Check the cell below "Auto generate codes"
                    elif term == 'Auto generate codes':
                        cell_below_auto_generate = sheet.cell(
                            row=3, column=term_index + 1
                        )
                        ...
                        if auto_code not in ['TRUE', 'FALSE']:
                            errors.append(
                                "Error: Value below 'Auto generate codes' should be 'TRUE' or 'FALSE'."
                            )
            ...
            for term in expected_terms:
                if term not in second_row_values:
                    ...
                    if term == 'Version':
                        ...
                        if str(cell_below_version.value) != version[1:]:
                            errors.append(
                                'Error: The version should be the same one indicated in the file name. Value found: {cell_below_version.value}'
                            )
                    ...
                    # Check the cell below "Code"
                        ...
                        if cell_below_code.value != code:
                            errors.append(
                                'Error: The code should be the same one indicated in the file name. Value found: {cell_below_code.value}'
                            )
                    ...
                    # Check the cell below "Description"
                        ...
                        if not description_pattern.match(cell_below_description.value):
                            errors.append(
                                "Error: Description should follow the schema: English Description + '//' + German Description. Value found: {cell_below_description.value}"
                            )
                ...
                if term not in second_row_values:
                    ...
                    if term == 'Version':
                        ...
                        if non_integer_cells:
                            # Append an error indicating the positions (row numbers) that are not integers
                            non_integer_indices = [
                                str(row) for row, _ in non_integer_cells
                            ]
                            invalid_values = [
                                str(value) for _, value in non_integer_cells
                            ]
                            errors.append(
                                f"Error: Values not valid found in the 'Version' column (they should be Integers) at row(s): {', '.join(non_integer_indices)}. Value(s) found: {', '.join(invalid_values)}"
                            )
                    ...
                    # Check the column below "Code"
                        ...
                        if invalid_codes:
                            ...
                            errors.append(
                                f"Error: Invalid code found in the '{term}' column at row(s): {', '.join(invalid_rows)}. Value(s) found: {', '.join(invalid_values)}"
                            )
                    ...
                    # Check the cell below "Description"
                        ...
                        if invalid_descriptions:
                            ...
                            invalid_values = [
                                str(value) for _, value in invalid_descriptions
                            ]
                            errors.append(
                                f"Error: Invalid value(s) found in the '{term}' column at row(s): {', '.join(invalid_rows)}. Description should follow the schema: English Description + '//' + German Description. Value(s) found: {', '.join(invalid_values)}"
                            )
                    ...
                    # Check the cell below "Mandatory"
                        ...
                        invalid_mandatory = [
                            (i + 3, cell.value)
                            for i, cell in enumerate(column_below_mandatory)
                            if cell.value not in ['TRUE', 'FALSE']
                        ]
                        if invalid_mandatory:
                            invalid_rows = [str(row) for row, _ in invalid_mandatory]
                            invalid_values = [
                                str(value) for _, value in invalid_mandatory
                            ]
                            errors.append(
                                f"Error: Invalid value found in the '{term}' column at row(s): {', '.join(invalid_rows)}. Accepted values: TRUE, FALSE. Value(s) found: {', '.join(invalid_values)}"
                            )
                    ...
                    # Check the cell below "Show in edit views"
                        ...
                        invalid_section = [
                            ...
                            for i, cell in enumerate(column_below_section)
                            if not re.match(
                                r'^[A-Z][a-z]*(?:\s[A-Z][a-z]*)*$', str(cell.value)
                            )
                        ]
                        if invalid_section:
                            invalid_rows = [str(row) for row, _ in invalid_section]
                            invalid_values = [
                                str(value) for _, value in invalid_section
                            ]
                            errors.append(
                                f"Error: Invalid value found in the '{term}' column at row(s): {', '.join(invalid_rows)}. Each word should start with a capital letter. Value(s) found: {', '.join(invalid_values)}"
                            )
                        ...
                        comments_seen = (
                            False  # Flag to track if "Comments" has been seen
                        )
                    ...
                    # Check the cell below "Property label"
                        ...
                        if invalid_label:
                            ...
                            errors.append(
                                f"Error: Invalid value found in the '{term}' column at row(s): {', '.join(invalid_rows)}. Specify the property label as text format. Value(s) found: {', '.join(invalid_values)}"
                            )
                        # Dynamically find the "Section" column
                        if 'Section' in second_row_values:
                            section_column_index = (
                                second_row_values.index('Section') + 1
                            )  # Find the index of the "Section" column
                            section_letter = index_to_excel_column(
                                section_column_index
                            )  # Convert index to Excel column letter
                            column_below_section = sheet[section_letter][
                                2:
                            ]  # Get all cells below the "Section" header

                            # New check for "Notes" in "Property label" and "Additional Information" in "Section"
                            for i, cell in enumerate(column_below_label):
                                if cell.value == 'Notes':
                                    section_value = column_below_section[
                                        i
                                    ].value  # Get the value in the "Section" column for the same row
                                    if section_value != 'Additional Information':
                                        errors.append(
                                            f"Error: 'Notes' found in the 'Property label' column at row {i + 5}, but corresponding 'Section' column does not contain 'Additional Information'. Value found: {section_value}"
                                        )
                    ...
                    # Check the cell below "Data type"
                        ...
                        if invalid_type:
                            ...
                            errors.append(
                                f"Error: Invalid value found in the '{term}' column at row(s): {', '.join(invalid_rows)}. Accepted types: INTEGER, REAL, VARCHAR, MULTILINE_VARCHAR, HYPERLINK, BOOLEAN, CONTROLLEDVOCABULARY, XML, TIMESTAMP, DATE, SAMPLE. Value(s) found: {', '.join(invalid_values)}"
                            )
                    ...
                    # Check the column below "Vocabulary code"
                        ...
                        invalid_vocab = [
                            ...
                            for i, cell in enumerate(column_below_vocab)
                            if cell.value is not None
                            and not re.match(r'^\$?[A-Z0-9_.]+$', str(cell.value))
                        ]
                        if invalid_vocab:
                            invalid_rows = [str(row) for row, _ in invalid_vocab]
                            invalid_values = [str(value) for _, value in invalid_vocab]
                            errors.append(
                                f"Error: Invalid vocabulary code found in the '{term}' column at row(s): {', '.join(invalid_rows)}. Value(s) found: {', '.join(invalid_values)}"
                            )
