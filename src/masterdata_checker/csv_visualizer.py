...
def generate_csv_and_download(o, instance):
    ...
    masterdata_headers = [
        ...
        f'EXPERIMENT TYPES ({len(experiment_types)})',
        f'OBJECT TYPES ({len(object_types)})',
        f'DATASET TYPES ({len(dataset_types)})',
        f'VOCABULARIES ({len(vocabs)})',
        f'PLUGINS ({len(plugins)})',
        f'MATERIAL TYPES ({len(material_types)})',
    ]
    ...
    filename = os.path.join(
        directory, f"{instance}_{datetime.now().strftime('%d%m%Y')}.csv"
    )
