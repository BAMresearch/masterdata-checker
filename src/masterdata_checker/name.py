...
def name_checker(file_path):
    ...
    if match:
        ...
        entity_type, entity_name, version, division, contact_person, extension = (
            match.groups()
        )
        ...
        if len(file_name) < 2:
            raise UserFailureException(
                'Error: Invalid file type. The file should be an Excel file (.xls or .xlsx)'
            )
            ...
            if len(file_parts) < 5:
                errors.append(
                    'Invalid name format. The name should contain different fields separated by underscores (_). Consult the wiki to see which ones.'
                )
                return ['\n'.join(errors), False]
            ...
            if not re.match(
                r'^(collection_type|object_type|dataset_type|vocabulary)$', etype
            ):
